...
class QuotesViewModelTests: XCTestCase {
    ...
    func testOuputSuccess() {
        let service = QuotesNetworkServiceMock(quotes: Quote.mockQuotes)
        let sut = QuotesViewModel(service: service)
    ...
    func testOuputFailure() {
        let service = QuotesNetworkServiceMock(quotes: nil)
        let sut = QuotesViewModel(service: service)
